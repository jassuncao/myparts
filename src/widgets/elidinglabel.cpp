...
ElidingLabel::ElidingLabel(const QString &text, QWidget *parent, Qt::WindowFlags f)
...
void ElidingLabel::paintEvent(QPaintEvent *event)
{
    ...
    if (fm.width(text()) > (contentsRect().width() - 8) && text().length() > 4) {
        QPainter p(this);
        QString elided_txt;
        elided_txt = fm.elidedText(text(), Qt::ElideRight, contentsRect().width() - 8,
                                   Qt::TextShowMnemonic);
        p.drawText(contentsRect(), Qt::TextSingleLine, elided_txt);
    } else
